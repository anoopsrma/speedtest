...
?>
    ...
    <?php
        ...
        if($_GET["op"]=="id"&&!empty($_GET["id"])){
            ...
            if($db_type=="mysql"){
                $q=$conn->prepare("select id,created_at,client_ip,isp_name,user_agent,resolution,download_speed,upload_speed,ping,jitter,dpi,perfomance from speedtest_users where id=?");
                $q->bind_param("i",$id);
                $q->execute();
                $q->store_result();
            ...
            if($db_type=="mysql"){
                $q=$conn->prepare("select id,created_at,client_ip,isp_name,user_agent,resolution,download_speed,upload_speed,ping,jitter,dpi,perfomance from speedtest_users order by created_at desc limit 0,100");
                $q->execute();
                $q->store_result();
                $q->bind_result($id,$timestamp,$ip,$ispinfo,$ua,$lang,$dl,$ul,$ping,$jitter,$log,$extra);
            } else if($db_type=="sqlite"||$db_type=="postgresql"){
    ...
    ?>
        <table>
            ...
            <tr><th>User agent and Resolution</th><td><?=$ua ?><br/><?=htmlspecialchars($lang, ENT_HTML5, 'UTF-8') ?></td></tr>
            ...
            <tr><th>DPI</th><td><?=htmlspecialchars($log, ENT_HTML5, 'UTF-8') ?></td></tr>
            <tr><th>Gpu</th><td><?=htmlspecialchars($extra, ENT_HTML5, 'UTF-8') ?></td></tr>
        </table>
    <?php
        }
    ?>
