...
if($db_type=="mysql"){
	...
	$q = $conn->prepare("select isp_name,download_speed,upload_speed,ping,jitter from speedtest_users where id=?");
	$q->bind_param("i",$id);
	$q->execute();
...
$ping=format($ping);
$jit=format($jit);
$isp_name=preg_replace("/^(\w+\s)/", "", $ispinfo);
