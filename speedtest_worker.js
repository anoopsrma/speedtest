...
var settings = {
	...
	height: 0, //extra data that can be passed to the telemetry through the settings
	width: 0, //extra data that can be passed to the telemetry through the settings
	dpi: 0, //extra data that can be passed to the telemetry through the settings
	battery: '', //extra data that can be passed to the telemetry through the settings
	perfomance: '' //extra data that can be passed to the telemetry through the settings
};
...
function sendTelemetry(done) {
	...
	console.log(telemetryIspInfo);

	try {
		var fd = new FormData();
		fd.append("download_speed", dlStatus);
		fd.append("upload_speed", ulStatus);
		fd.append("ping", pingStatus);
		fd.append("jitter", jitterStatus);
		fd.append("client_ip", telemetryIspInfo.rawIspInfo.ip);
		fd.append("isp_name", telemetryIspInfo.rawIspInfo.org);
		fd.append("latitude", telemetryIspInfo.rawIspInfo.loc.split(',')[0]);
		fd.append("longitude", telemetryIspInfo.rawIspInfo.loc.split(',')[1]);
		fd.append("resolution", settings.width+'*'+settings.height);
		fd.append("dpi", settings.dpi);
		fd.append("battery", settings.battery);
		fd.append("perfomance", settings.perfomance);
